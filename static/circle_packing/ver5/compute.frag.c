...
void main() {
  ...
  if (u_opcode == OPCODE_TEST_HIT) {
    ...
    for (int i = -1 * r; i < r; i++) {
      for (int j = -1 * r; j < r; j++) {
        vec4 test =
          u_iteration * texelFetch(u_pixel_texture, ivec2(color_out.xy) + ivec2(i, j), 0);
        if (test.b > 0.) {
          failed = true;
          break;
        }
    ...
    color_out = color + u_iteration * texelFetch(u_pixel_texture, ivec2(coords), 0);
    ...
    if (found) {
      ...
      vec4 o_color = u_iteration * texelFetch(u_pixel_texture, ivec2(coords), 0);
      if (o_color.a == 0.) {
